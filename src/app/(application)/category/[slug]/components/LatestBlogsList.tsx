...
import { CategoryBlogsResponse } from '@/types/blog.types';
import { useInfiniteQuery } from '@tanstack/react-query';
import React, { useEffect } from 'react';
import { useInView } from 'react-intersection-observer';

type BlogListProps = {
  initialData: ActionResponse<CategoryBlogsResponse>;
  categoryId: string;
};

function LatestBlogsList({ initialData, categoryId }: BlogListProps) {
  ...
  const {
    ...
    queryKey: ['blogs', 'latest', { categoryId }],
    queryFn: async ({ pageParam }) => {
      const result = await getCategoryLatestBlogs({
        categoryId,
        page: pageParam,
      });
